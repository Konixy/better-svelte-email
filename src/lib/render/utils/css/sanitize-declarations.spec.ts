...
`);
	...
	test('rgba space syntax to comma syntax conversion', () => {
		...
		sanitizeDeclarations(stylesheet);
		expect(generate(stylesheet), 'conversion with space syntax and no alpha').toMatchSnapshot();

		stylesheet = parse('div { color: rgb(255 0 128 / 0.5); }');
		sanitizeDeclarations(stylesheet);
		expect(generate(stylesheet), 'conversion with space syntax and alpha').toMatchSnapshot();

		stylesheet = parse('div { color: rgb(100% 0% 50% / 50%); }');
		sanitizeDeclarations(stylesheet);
		expect(generate(stylesheet), 'conversion with percentage values').toMatchSnapshot();
		...
		stylesheet = parse('div { background: linear-gradient(rgb(255 0 0), rgb(0 255 0 / 0.8)); }');
		sanitizeDeclarations(stylesheet);
		expect(generate(stylesheet)).toMatchSnapshot();
		...
		sanitizeDeclarations(stylesheet);
		expect(generate(stylesheet), 'treatment for already supported rgb syntax').toMatchSnapshot();
	});
	...
	test('hex to rgb conversion', () => {
		...
		stylesheet = parse('div { background: linear-gradient(#ff0000, #00ff00, #0000ff); }');
		sanitizeDeclarations(stylesheet);
		expect(generate(stylesheet)).toMatchSnapshot();
		...
		stylesheet = parse('div { content: "Visit our site at example.com#section"; }');
		sanitizeDeclarations(stylesheet);
		expect(generate(stylesheet)).toMatchSnapshot();
