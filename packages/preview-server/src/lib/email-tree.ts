...
export function filterEmailTree(
	nodes: readonly EmailTreeEntry[],
	...
	return nodes.flatMap((node) => {
		...
		const filteredItems = filterEmailTree(node.items, normalizedQuery);
		if (node.path.toLowerCase().includes(normalizedQuery) || filteredItems.length > 0) {
